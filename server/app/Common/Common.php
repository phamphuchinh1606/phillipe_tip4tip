    ...
    public static function sendMailChangeStatus($status, $tipster_id = 1, $lead_id = 1, $product_id = 1, $points_new = 0){
        try{
            ...
            if($tipster->preferred_lang == 'vn'){
                ...
                $emailTemplate = "messagetemplates.emails.example_vn";
            }else{
                $title = $template->subject_en;
                $content = $template->content_en;
                $emailTemplate = "";
                $emailTemplate = "messagetemplates.emails.example_en";
            }
            $lead_name = "";
            if(asset($lead)){
...
//            Send mail tipster application
            $messageSendTipster['author'] = Auth::user()->id;
            $messageSendTipster['title'] = $title;
            $messageSendTipster['content'] = $content;
            $messageSendTipster['delete_is'] = 0;
            $messageSendTipster['read_is'] = 0;
            $messageSendTipster['receiver'] = $tipster_id;
            Message::create($messageSendTipster);

            return Mail::send($emailTemplate, $data, function($message) use ($emailTo, $subjectTo, $tipster_name) {
